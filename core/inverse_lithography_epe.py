...
class EPEInverseLithographyOptimizer:
    ...
    # 修改_compute_analytical_gradient方法，使其同时返回EPE和PE损失
    def _compute_analytical_gradient(self, mask, target, epsilon=1e-10):
        """
        EPE梯度计算，同时返回EPE和PE损失
        """
        # 1. 前向传播
    ...
    def optimize(self, initial_mask, target, learning_rate=None,
        ...
        if log_csv:
            ...
            # 计算初始损失
            initial_epe_loss, initial_pe_loss, _, _, _ = self._compute_analytical_gradient(initial_mask, target)
            ...
            csv_logger.start_logging(
                ...
                initial_loss=initial_epe_loss,  # 使用EPE损失作为初始损失
        ...
        if csv_logger:
            print(f"Logging to: {csv_logger.filepath}")
        ...
        for iteration in range(max_iterations):
            # 使用解析梯度计算损失和梯度
            epe_loss, pe_loss, gradient, aerial_image, printed_image = self._compute_analytical_gradient(mask, target)

            # 记录两种损失
            history['epe_loss'].append(epe_loss)
            history['pe_loss'].append(pe_loss)
            history['loss'].append(epe_loss)  # 主损失是EPE损失
            ...
            if csv_logger:
                ...
                # 注意：CSV日志只记录主损失（EPE损失）
                csv_logger.log_iteration(
                    iteration=iteration,
                    loss=epe_loss,  # 只记录EPE损失到CSV
                    gradient=gradient,
                    mask=mask,
            ...
            if iteration % 10 == 0:
                ...
                if self.optimizer_type == 'momentum':
                    ...
                    print(
                        f"Iteration {iteration}: EPE Loss = {epe_loss:.6f}, PE Loss = {pe_loss:.6f}, "
                        f"Grad Norm = {grad_norm:.6f}, Velocity Norm = {velocity_norm:.6f}")
                elif self.optimizer_type == 'adam':
                    print(
                        f"Iteration {iteration}: EPE Loss = {epe_loss:.6f}, PE Loss = {pe_loss:.6f}, "
                        f"Grad Norm = {grad_norm:.6f}, t = {self.optimizer_state['t']}")
                else:
                    print(f"Iteration {iteration}: EPE Loss = {epe_loss:.6f}, PE Loss = {pe_loss:.6f}, "
                          f"Grad Norm = {grad_norm:.6f}")

            # 早期停止
            if epe_loss < 1e-6 and iteration > 20:
        ...
        print(f"EPE Optimization completed in {total_time:.2f}s. Best EPE loss: {best_epe_loss:.6f}, "
              f"Best PE loss: {best_pe_loss:.6f}")

        # 将日志路径添加到history中
        if csv_logger:
            history['csv_log_path'] = csv_logger.filepath
...
def inverse_lithography_optimization_epe(initial_mask, target_image,
    ...
    参数:
    - optimizer_type: 优化器类型 ('momentum', 'momentum', 'rmsprop', 'cg', 'adam')
